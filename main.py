...
def self_train(env, agent1, agent2, num_episodes):
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % 100 == 0:
            avg_r1 = np.mean(episode_rewards1[-100:])
            avg_r2 = np.mean(episode_rewards2[-100:])
            print(f"Model 1 Average Reward (Episode {episode-99}-{episode+1}): {avg_r1}")
            print(f"Model 2 Average Reward (Episode {episode-99}-{episode+1}): {avg_r2}")
